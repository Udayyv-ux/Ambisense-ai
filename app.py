...
def load_balanced_data():
    ...
    df = pd.read_csv(io.StringIO(requests.get(url).text), sep='\t', names=['label', 'message'])
...
st.sidebar.info("This AI uses a Bidirectional LSTM network to detect semantic patterns in spam messages.")
...
with tab1:
    ...
    with col1:
        input_text = st.text_area("Message Content", height=200, placeholder="Paste email or SMS content here...")
        analyze_btn = st.button("RUN SECURITY SCAN")
    
    with col2:
        st.markdown("### Scan Results")
        if analyze_btn and input_text:
            cleaned = " ".join([ps.stem(w) for w in re.sub('[^a-zA-Z]', ' ', input_text).lower().split() if w not in set(stopwords.words('english'))])
            ...
            st.info("Waiting for input... Click scan to begin.")
